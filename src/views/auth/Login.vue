<template>
  <AuthLayout>
    <div class="flex min-h-full items-center justify-center px-6 py-12 lg:px-8 font-mono">
      <div class="bg-white rounded-xl border p-6 border-slate-200 mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        <form @submit.prevent="singinWithEmailAndPassword" class="space-y-6">
          <div>
            ...
            <div class="mt-2">
              <input v-model="email.value.value" type="email" name="email" id="email" autocomplete="email"
          ...
          <div>
            ...
            <div class="mt-2">
              <input v-model="password.value.value" type="password" name="password" id="password"
...
const singinWithEmailAndPassword = handleSubmit(async () => {
  try {
